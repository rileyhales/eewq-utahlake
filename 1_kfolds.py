...
for alpha in range(1, 101):
    ...
    coefficients = []
    iteration_metrics = []
    iter_metrics_labels = ['me_test', 'mse_test', 'sse_test', 'me_train', 'mse_train', 'sse_train']
    ...
    for i, (train_index, test_index) in enumerate(rkfolder.split(X)):
        ...
        test_sse = np.sum(np.square(y_test - pred_test))

        train_mse = mean_squared_error(y_train, pred_train)
        train_me = np.mean(y_train - pred_train)
        train_sse = np.sum(np.square(y_test - pred_test))
